...
def get_img_label(p, size, linetype=1):
    ...
    w = labelY.shape[1]
    c = labelY.shape[2]
    labelY = labelY.reshape((c, h, w))
    h = lines.shape[0]
    w = lines.shape[1]
    c = lines.shape[2]
    lines = lines.reshape((c, h, w))
    return img, lines, labelY
...
def get_random_data(image, lines, labels, size=(1024, 1024), jitter=.3, hue=.1, sat=1.5, val=1.5):
    ...
    image_data = image_data.reshape((c, h ,w))# numpy array, 0 to 1
    N = len(lines)
    for i in range(N):
        p1, p2 = lines[i]
...
def gen(paths, batchsize=2, linetype=2):
    ...
    while True:
        ...
        X = np.zeros((batchsize, 3, size, size)).astype('float32')
        Y = np.zeros((batchsize, 2, size, size)).astype('float32')
